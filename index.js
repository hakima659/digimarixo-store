...
async function initDB(env) {
  ...
  /*
    ...
    If category or stock already exists, nothing happens.
  */
  ...
  if (!names.has("category")) {
    await env.DB
      .prepare(`ALTER TABLE products ADD COLUMN category TEXT DEFAULT ''`)
      .run();
  }

  if (!names.has("stock")) {
    await env.DB
      .prepare(`ALTER TABLE products ADD COLUMN stock INTEGER DEFAULT 0`)
      .run();
  }

  if (!names.has("image")) {
    await env.DB
      .prepare(`ALTER TABLE products ADD COLUMN image TEXT DEFAULT ''`)
      .run();
  }

  if (!names.has("active")) {
    await env.DB
      .prepare(`ALTER TABLE products ADD COLUMN active INTEGER DEFAULT 1`)
      .run();
  }
...
async function createOrder(request, env) {
  ...
  const order = await env.DB.prepare(`
    INSERT INTO orders
    (customer_name, customer_email, customer_phone, address, total, status)
    VALUES (?, ?, ?, ?, ?, 'pending')
  `)
    .bind(
      customerName,
  ...
  return layout(
    ...
    <section class="hero">
      ...
      <div class="hero-card">
        ...
        <h3>DigiMarixo</h3>
        <p>
          انتخاب، خرید و مدیریت محصولات در یک فروشگاه مدرن.
        </p>
      </div>
    </section>
    ...
    <section class="features">
      ...
      <div class="feature">
        <div class="feature-icon">◆</div>
        <h3>دیجیتال</h3>
        <p>تمرکز فروشگاه بر محصولات دیجیتال و کاربردی.</p>
      </div>
    ...
    <section class="promo">
      <div>
        ...
        <p>
          محصولات را بررسی کنید و از طریق حساب کاربری
...
function productCard(product) {
  ...
  const description = escapeHTML(
    product.description || "محصول دیجیتال دیجی‌ماریکسو"
  );
            ...
            stock > 0
              ? `
                <button
                  ...
                  onclick="addToCart(${id}, '${escapeJS(product.name || "محصول")}', ${Number(product.price || 0)})"
                >
                  افزودن به سبد
                </button>
              `
...
function accountPage() {
  return layout(
    ...
    <section class="account-wrap">
      ...
      <div class="account-card register">
        <h2>ایجاد حساب</h2>
...
function adminPage() {
  return layout(
    ...
    <section class="page-title">
      ...
      <p>
        پنل مدیریت فروشگاه در حال آماده‌سازی است.
      </p>
    </section>
...
<script>
  ...
  function renderCart() {
    ...
    box.innerHTML = cart.map(function(item, index) {
      ...
      return (
        '<div style="padding:15px 0;border-bottom:1px solid #e2e8f0;display:flex;justify-content:space-between;gap:15px;align-items:center">' +
          '<div>' +
            '<strong>' +
              item.name +
            '</strong>' +
          ...
          '<div style="text-align:left">' +
            ...
            '</strong>' +
            '<br>' +
            '<button class="btn small orange" onclick="removeCartItem(' +
              index +
            ')">حذف</button>' +
          '</div>' +
        '</div>'
